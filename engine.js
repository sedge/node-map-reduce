...
export class Engine {
  ...
  async run(jobId, payload) {
    ...
    while (payload.length) {
      ...
      while (promises.length < 10 && payload.length) {
        const partialPayload = payload.splice(0, config.get('engine_chunk_size'));
        promises.push(this.map(jobId, partialPayload))
      }

      const partialResults = await Bluebird.all(promises);

      mapResults.push(...partialResults);
    }
    ...
    while (finalGroupedResults.length) {
      ...
      while (promises.length < 10 && finalGroupedResults.length) {
        const partialPayload = finalGroupedResults.splice(0, config.get('engine_chunk_size'));
        promises.push(
          this.reduce(jobId, partialPayload)
        );
      }

      const partialResults = await Bluebird.all(promises);

      reduceResults.push(...partialResults);
    }

    return reduceResults;
  }

  async map(jobId, payload) {
    ...
    const fork = cp.fork(`${__dirname}/mapper.js`, [jobId.toString()], {
      env: process.env
    });
  ...
  async reduce(jobId, payload) {
    ...
    const fork = cp.fork(`${__dirname}/reducer.js`, [jobId.toString(), chunkId.toString()]);
